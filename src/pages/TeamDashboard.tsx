...
export default function TeamDashboard() {
  ...
  const handleBan = () => {
    logout();
    alert('You have been disqualified for violating the rules.');
    window.close();
  ...
  const useLifeline = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert('Lifeline activated! Browser restrictions disabled for 5 minutes.');
        setRestrictionsDisabled(true);
        setTimeout(() => {
          setRestrictionsDisabled(false);
          alert('Lifeline expired. Browser restrictions re-enabled.');
        }, 5 * 60 * 1000); // 5 minutes
        fetchStatus();
      } else {
        const data = await res.json();
      ...
      <header className="bg-neutral-900 border-b border-neutral-800 p-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 text-neutral-300">
            ...
            <button 
              onClick={useLifeline}
              disabled={!status.lifeline || status.lifeline.lifeline_remaining === 0}
